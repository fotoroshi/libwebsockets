...
append_string:
	...
	if (!args->jose->e[ctx->path_match - 1].buf) {
		args->jose->e[ctx->path_match - 1].buf = args->temp;
...
static int
lws_jose_parse(struct lws_jose *jose,
	       const uint8_t *buf, int n,
	       uint8_t *temp, int temp_len, int is_jwe)
{
...
lws_jws_parse_jose(struct lws_jose *jose,
		   const char *buf, int len, uint8_t *temp, int temp_len)
{
