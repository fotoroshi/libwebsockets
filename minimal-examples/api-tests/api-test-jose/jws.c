...
test_jws_none(struct lws_context *context)
{
	...
	if (lws_jws_parse_jose(&jose, map.buf[LJWS_JOSE],
			       map.len[LJWS_JOSE], temp, sizeof(temp)) < 0) {
...
test_jws_HS256(struct lws_context *context)
{
	...
	uint8_t digest[LWS_GENHASH_LARGEST], temp[256];
	...
	/* parse the JOSE header */
	if (lws_jws_parse_jose(&jose, test1, strlen(test1), temp, sizeof(temp)) < 0) {
		lwsl_err("%s: JOSE parse failed\n", __func__);
		goto bail;
	}
...
test_jws_ES512(struct lws_context *context)
{
	...
	/* parse the JOSE header */
	if (lws_jws_parse_jose(&jose, jws.map.buf[LJWS_JOSE],
			      jws.map.len[LJWS_JOSE], temp, sizeof(temp)) < 0) {
		lwsl_err("%s: JOSE parse failed\n", __func__);
		goto bail;
	}
	...
	buf[l] = '\0';
	if (lws_jws_sig_confirm_compact_b64(buf, l, &jwk, context) < 0) {
		lwsl_notice("%s: confirm new ecdsa sig failed\n", __func__);
		goto bail1;
	}
